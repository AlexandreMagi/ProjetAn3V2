...
public class Swarmer : Enemy<DataSwarmer>, IGravityAffect, ISpecialEffects
{
    ...
    public void OnPull(Vector3 position, float force, bool isReppelForce = false, Vector3? normalOfReppel = null)
    {
        if (!ignoresAllGravityAffects)
        {
            ...
            ReactGravity<DataSwarmer>.DoPull(rbBody, position, force, isInTheAir, isReppelForce, normalOfReppel);
    ...
    protected override void Die()
    {
        if (!isDying)
        {
            ...
            if (CameraHandler.Instance.GetDistanceWithCam(transform.position) > entityData.distanceMinWithCamToPlayVFX)
            {
                ...
                Vector3 camPos = CameraHandler.Instance != null ? CameraHandler.Instance.GetCurrentCam().transform.position : Camera.main.transform.position;
