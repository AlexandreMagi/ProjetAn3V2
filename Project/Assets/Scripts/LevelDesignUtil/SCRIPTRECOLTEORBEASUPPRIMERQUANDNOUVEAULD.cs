...
public class SCRIPTRECOLTEORBEASUPPRIMERQUANDNOUVEAULD : MonoBehaviour, IBulletAffect
{
    ...
    void Update()
    {
        ...
        else if (!bItemDestroyed)
        {
            ...
            CustomSoundManager.Instance.PlaySound("GravityOrbOvercharge_Boosted", "Effect", 0.2f);
        }
    ...
    void SecondShake()
    {
        ...
        CustomSoundManager.Instance.PlaySound("EquipOrb_Boosted", "Effect", 1f);
    ...
    public void PlayerShootOnObjet(float Dmg)
    {
        if (bPlayerCanDammage && !bItemDestroyed)
        {
            if (currentTimer == 0)
            {
                ...
                CustomSoundManager.Instance.PlaySound("ImpactOrbeSequence_Boosted", "Effect", 1f);
            }
