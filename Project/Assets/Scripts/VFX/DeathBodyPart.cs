...
public class DeathBodyPart : MonoBehaviour
{
    ...
    public void CheckIfMustPop(Vector3 pos)
    {
        if (!forceNoSpawn)
        {
            ...
            else if (isAlwaysSpawning && isEnemyPart) Pop(pos);
        }
    }

    void Pop(Vector3 posInit)
    {
        gameObject.SetActive(true);
        timerBeforeDisapear = Random.Range(minTimerBeforeDisapear, maxTimerBeforeDisapear);
        transform.rotation = Random.rotation;
        transform.position = posInit + Random.insideUnitSphere + Vector3.up*0.5f;
        Rigidbody rb;
